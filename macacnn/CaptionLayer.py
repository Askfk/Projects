...
def reset_state(rois, units, tokenizer):
    tile_nums = units // rois.shape[-1]
    hidden = tf.zeros_like(rois)
    # [batch, num_rois, units]
    hidden = tf.tile(hidden, [1, 1, tile_nums])

    # [batch, num_rois]
    dec_input = tf.ones_like(rois[:, :, 0])
...
def build_caption_layer_graph(rois, feature_maps, image_meta, pool_size,
                              tokenizer, mode, config, target_caption=None):
    """
    ...
    # print('captionlayer')
    # print(rois.shape)
    # processed_x = tf.reshape(x, [x.shape[0], -1, pool_size ** 2, x.shape[4]])
    processed_x = tf.keras.layers.Lambda(lambda x: tf.reshape(x, [x.shape[0], -1, pool_size ** 2, x.shape[4]]))(x)
    ...
    if mode == 'training':
        ...
        outputs = tf.keras.layers.Lambda(lambda x: tf.stack(x, axis=0))(outputs)
        # outputs = tf.stack(outputs, axis=0)
        # shape == [batch, num_rois, MAX_LENGTH, vocab_size]
        outputs = tf.keras.layers.Lambda(lambda x: tf.transpose(x, perm=[1, 2, 0, 3]))(outputs)
        ...
        attention_weights = tf.keras.layers.Lambda(lambda x: tf.stack(x, axis=0))(attention_weights)
