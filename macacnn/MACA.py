...
from macacnn.backbone import build_backbone_net_graph
from macacnn.RPNModel import build_rpn_model
from macacnn.ProposalLayer import ProposalLayer
from macacnn.FPNHeads import fpn_bbox_graph, build_fpn_mask_graph
from macacnn.DetectionTargetLayer import DetectionTargetLayer
from macacnn.CaptionDetectionTargetLayer import CaptionDetectionTargetLayer
from macacnn.CaptionLayer import build_caption_layer_graph
from macacnn.DataGenerator import DataGenerator

from macacnn import losses, utils, utils_graph

from macacnn.CaptionDetectionLayer import CaptionDetectionLayer
...
class MACA():
    ...
    def build(self, mode, config):
        ...
        proposal_layer = ProposalLayer(
            ...
            config=config,
            name="ROI")
        mask_rpn_rois = proposal_layer([mask_rpn_class, mask_rpn_bbox, anchors])
        caption_rpn_rois = proposal_layer([caption_rpn_class, caption_rpn_bbox, anchors])
        # mask_rpn_rois = ProposalLayer(
        #     proposal_count=mask_proposal_count,
        #     nms_threshold=config.RPN_NMS_THRESHOLD,
        #     task='mask',
        #     config=config,
        #     name="mask_ROI")([mask_rpn_class, mask_rpn_bbox, anchors])
        # caption_rpn_rois = ProposalLayer(
        #     proposal_count=caption_proposal_count,
        #     nms_threshold=config.RPN_NMS_THRESHOLD,
        #     task='caption',
        #     config=config,
        #     name="caption_ROI")([caption_rpn_class, caption_rpn_bbox, anchors])
        ...
        if mode == 'training':
            ...
            mask_rois, target_class_ids, mask_target_bbox, target_mask = \
                DetectionTargetLayer(config, name='mask_proposal_targets')([
                    mask_target_rois, input_gt_class_ids, mask_gt_boxes, input_gt_masks])

            caption_rois, caption_target_bbox, target_caption, caption_target_bbox_scores = \
            ...
            macacnn_bbox, bbox_scores = fpn_bbox_graph(caption_rois,
                                                       mrcnn_feature_maps,
                                                       input_image_meta,
                                                       self.config.POOL_SIZE,
                                                       train_bn=config.TRAIN_BN)
            macacnn_mask = build_fpn_mask_graph(mask_rois,
                                                mrcnn_feature_maps,
                                                input_image_meta,
                                                config.MASK_POOL_SIZE,
                                                config.NUM_CLASSES,
                                                train_bn=config.TRAIN_BN)
            macacnn_caption, _ = build_caption_layer_graph(caption_rois,
                                                           mrcnn_feature_maps,
                                                           input_image_meta,
                                                           config.POOL_SIZE,
                                                           self.tokenizer,
            ...
            bbox_loss = keras.layers.Lambda(lambda x: losses.mrcnn_bbox_loss_graph(*x), name="macacnn_bbox_loss")(
                [caption_target_bbox, macacnn_bbox])
            bbox_score_loss = keras.layers.Lambda(lambda x: losses.macacnn_bbox_score_loss_graph(*x),
                                                  name='macarcnn_bbox_score_loss')([
                caption_target_bbox_scores, bbox_scores])
            ...
            outputs = [mask_rpn_class_logits, caption_rpn_class_logits,
                       ...
                       macacnn_bbox, macacnn_mask, macacnn_caption,
                       ...
                       bbox_loss, bbox_score_loss, mask_loss, caption_loss]
            model = keras.Model(inputs, outputs, name='macacnn')
        else:
            macacnn_bbox, bbox_scores = fpn_bbox_graph(caption_rpn_rois,
                                                       mrcnn_feature_maps,
                                                       input_image_meta,
                                                       config.POOL_SIZE,
                                                       train_bn=config.TRAIN_BN)

            detections = CaptionDetectionLayer(self.config, name='macacnn_caption_detection')([
                caption_rpn_rois, bbox_scores, macacnn_bbox, input_image_meta])

            detection_boxes = keras.layers.Lambda(lambda x: x[..., :4])(detections)

            macacnn_caption, attention_weights = build_caption_layer_graph(detection_boxes,
                                                                           mrcnn_feature_maps,
                                                                           input_image_meta,
                                                                           self.config.POOL_SIZE,
                                                                           self.tokenizer,
            ...
            macacnn_mask = build_fpn_mask_graph(detection_boxes,
                                                mrcnn_feature_maps,
                                                input_image_meta,
                                                config.MASK_POOL_SIZE,
                                                config.NUM_CLASSES,
            ...
            model = keras.Model([input_image, input_image_meta, input_anchors],
                                [detections, macacnn_bbox, macacnn_caption, attention_weights, macacnn_mask,
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = [
            "mask_rpn_class_loss", "caption_rpn_class_loss",
            "mask_rpn_bbox_loss", "caption_rpn_bbox_loss",
            "macacnn_bbox_loss", "macarcnn_bbox_score_loss",
    ...
    def unmold_detections(self, detections, maca_masks, maca_captions, original_image_shape,
                          image_shape, window):
        ...
        maca_mask: [N, height, width, num_classes]
        ...
        return boxes, scores, full_masks, captions
...
if __name__ == '__main__':
    ...
    # model.model.summary()
    # model.model.save('/Users/liyiming/Desktop/Birmingham Life/project/yxl1215/code/version1/Mask-Dence-Cap-R-CNN/macacnn')
