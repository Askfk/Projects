...
from macacnn import utils, utils_graph
import numpy as np
from tensorflow import keras

from macacnn.utils import compute_backbone_shapes
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    caption_boxes, caption_boxes_scores = dataset.load_caption_boxes(image_id)

    return image, image_meta, class_ids, bbox, mask, captions, caption_boxes, caption_boxes_scores
...
def build_mask_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
...
def build_caption_rpn_targets(image_shape, anchors, gt_boxes, config):
    """Given the anchors and GT boxes, compute overlaps and identify positive
    anchors and deltas to refine them to match their corresponding GT boxes.
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        while b < self.batch_size:
            ...
            image, image_meta, gt_class_ids, gt_boxes_mask, gt_masks, gt_captions, \
            gt_boxes_caption, gt_boxes_caption_scores = load_image_gt(self.dataset, self.config, image_id,
            ...
            # Mask RPN Targets
            mask_rpn_match, mask_rpn_bbox = build_mask_rpn_targets(image.shape, self.anchors,
                                                                   gt_class_ids, gt_boxes_mask, self.config)

            # Caption RPN Targets
            caption_rpn_match, caption_rpn_bbox = build_caption_rpn_targets(image.shape, self.anchors,
                                                                            gt_boxes_caption,
                                                                            self.config)
            ...
            if gt_boxes_caption.shape[0] > self.config.MAX_GT_INSTANCES:
                ...
                gt_boxes_caption_scores = gt_boxes_caption_scores[ids]
            ...
            batch_caption_gt_boxes_scores[b, :gt_boxes_caption_scores.shape[0]] = gt_boxes_caption_scores
            # if self.random_rois:
        ...
        inputs = [batch_images, batch_image_meta,
                  ...
                  batch_gt_class_ids, batch_mask_gt_boxes, batch_gt_masks, batch_caption_gt_boxes, batch_gt_captions,
...
if __name__ == '__main__':
    from macacnn.demo.cocovg import CocoVGDataset, CocoVGConfig
    config = CocoVGConfig()
