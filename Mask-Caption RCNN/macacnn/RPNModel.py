...
def rpn_graph(feature_map, anchors_per_location, anchor_stride, task):
    """
...
def build_rpn_model(anchor_stride, anchors_per_location, depth, task):
    ...
    input_feature_map = keras.layers.Input(shape=[None, None, depth],
                                           name='{}_input_rpn_feature_map'.format(task))
    outputs = rpn_graph(input_feature_map, anchors_per_location, anchor_stride, task)
    return keras.Model([input_feature_map], outputs, name='{}_rpn_model'.format(task))
